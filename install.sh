...
pushd "${jla_tool_path}" >/dev/null || exit
echo " # Found jla_tools at: ${jla_tool_path}"
...
if (pip -vvv freeze -r requirements.txt 2>&1 | grep "not installed") >/dev/null 2>&1; then
    echo " # Installing python dependencies for jla_tool.."
...
tag_begin="# JLA_TOOL_AUTOGEN_SOURCE_BEGIN ##"
tag_end="## JLA_TOOL_AUTOGEN_SOURCE_END ##"

# Delete the section if it exists
if grep -q "${tag_end}" ~/.bashrc
then
    echo " # Removing old jla_tools from bashrc.."
    file_name=~/.bashrc
...
# Re-add the section
echo " # Adding jla_tools to bashrc.."
echo "
${tag_begin}
  if [ -f ${jla_tool_path}/env.sh ]; then
    source ${jla_tool_path}/env.sh
  else
    echo 'Failed to source ${jla_tool_path}/env.sh'
  fi
${tag_end}
" >> ~/.bashrc
